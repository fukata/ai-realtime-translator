...
class RnnoiseProcessor extends AudioWorkletProcessor {
  ...
  private in48: Float32Array = new Float32Array(0);
  private out48: Float32Array = new Float32Array(0);
  // Resampler state: source (context rate) -> 48k
  private rsSrc: Float32Array = new Float32Array(0);
  private rsPos = 0; // fractional index in rsSrc
  private rsEnabled = false;
  private rsStepUp = 1; // src per one 48k output sample (in samples)
  // Downsampler state: 48k -> context rate
  private dsPos = 0; // fractional index in out48
  private dsStepDown = 1; // 48k samples per one context output sample
  ...
  constructor() {
    ...
    if (sampleRate !== 48000) {
      ...
      try { this.port.postMessage({ type: 'rnnoise.status', status: 'resampling' }); } catch {}
    } else {
  ...
  process(inputs: Float32Array[][], outputs: Float32Array[][]) {
    ...
    if (this.rsEnabled) {
      ...
      for (let i = produced; i < needed; i++) out[i] = inCh ? inCh[i] : 0;
    } else {
      if (this.out48.length >= n) {
        ...
        if (have > 0) outCh.set(this.out48.subarray(0, have), 0);
